(() => {
    var Z = Object.create,
        ...
        oe = (s, n, p) => {
            if ((n && typeof n == 'object') || typeof n == 'function')
                for (let d of re(n)) !te.call(s, d) && d !== 'default' && L(s, d, { get: () => n[d], enumerable: !(p = ne(n, d)) || p.enumerable });
            return s;
        },
        se = (s) =>
            oe(
                ae(
                    L(
                        s != null ? Z(ee(s)) : {},
                        'default',
                        s && s.__esModule && 'default' in s ? { get: () => s.default, enumerable: !0 } : { value: s, enumerable: !0 },
                    ),
                ),
                s,
            ),
        fe = ie((s, n) => {
            (function (p, d, M) {
                ...
                function T(e) {
                    ...
                    return h[e.which] ? h[e.which] : y[e.which] ? y[e.which] : String.fromCharCode(e.which).toLowerCase();
                }
                function V(e, t) {
                ...
                function I(e, t) {
                    ...
                    for (a = X(e), b = 0; b < a.length; ++b)
                        (c = a[b]), q[c] && (c = q[c]), t && t != 'keypress' && g[c] && ((c = g[c]), P.push('shift')), O(c) && P.push(c);
    ...
    document.addEventListener('alpine:init', () => {
        ...
        window.Alpine.store(
            'theme',
            s === 'dark' || (s === 'system' && window.matchMedia('(prefers-color-scheme: dark)').matches) ? 'dark' : 'light',
        ),
            window.addEventListener('theme-changed', (n) => {
                let p = n.detail;
                localStorage.setItem('theme', p),
                    p === 'system' && (p = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'),
...
document.addEventListener('DOMContentLoaded', function () {
    // Check if we're on a page with CKEditor
    if (typeof CKEDITOR !== 'undefined') {
        // Setup a file upload adapter
        CKEDITOR.on('instanceReady', function (ev) {
            const editor = ev.editor;

            // Log when CKEditor is ready
            console.log('CKEditor instance ready:', editor.name);

            // Set up the upload URL
            editor.config.filebrowserUploadUrl = '/ckeditor/upload';
            editor.config.filebrowserUploadMethod = 'form';
            editor.config.uploadUrl = '/ckeditor/upload';

            // Custom image upload handling
            editor.on('fileUploadRequest', function (evt) {
                console.log('File upload request triggered');
            ...
            editor.on('fileUploadResponse', function (evt) {
                ...
                try {
                    ...
                    if (response.uploaded) {
                        ...
                        console.log('Upload successful, image URL:', response.url);
                    } else {
                        evt.cancel();
                        console.error('Upload failed:', response.error.message);

                        // Show error in editor
                        editor.showNotification('Upload failed: ' + response.error.message, 'warning');
                    }
                } catch (e) {
                    evt.cancel();
                    console.error('Error parsing response:', e);
                    editor.showNotification('Invalid server response', 'warning');
                }
